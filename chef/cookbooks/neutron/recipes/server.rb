...
# backend here. "apic_aim" is supported on OpenStack Newton release too.
...
use_apic && pkgs += node[:neutron][:platform][:cisco_apic_pkgs]
...
all_plugin_snippets = [node[:neutron][:ml2_config_file],
                       node[:neutron][:nsx_config_file]]
...
when "ml2"
  ...
  if ml2_mechanism_drivers.include?("apic_aim")
    ml2_extension_drivers.unshift("apic_aim")
  end
  if use_hyperv
...
if use_apic
  gbp_db_manage_command = "gbp-db-manage --config-file #{node[:neutron][:config_file]} upgrade head"
  aim_db_manage_command = "aimctl --config-file #{node[:neutron][:config_file]} db-migration upgrade head"
