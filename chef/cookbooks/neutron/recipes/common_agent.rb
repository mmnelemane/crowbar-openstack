...
if neutron[:neutron][:networking_plugin] == "ml2"
  ...
  when ml2_mech_drivers.include?("cisco_apic_ml2")
    ["openvswitch-gbp", "openvswitch-gbp-switch", "openvswitch-gbp-kmp-default"].each { |p| package p }

    # Install additional packages on compute node
    if node.roles.include?("nova-compute-kvm")
      node[:neutron][:platform][:cisco_apic_pkgs].each { |p| package p }
      ["supervisor", "agent-ovs", "lldpd"].each { |p| package p }
    end
